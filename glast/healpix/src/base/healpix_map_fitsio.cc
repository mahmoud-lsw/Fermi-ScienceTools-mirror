...
template<typename T> void read_Healpix_map_from_fits
  ...
  {
  string ordering;
#if _MSC_VER == 1300                     //VS 2003 compiler version
    inp.get_key ("ORDERING", ordering);  //breaks VS 2003 compiler
#endif
  arr<T> myarr;
  inp.read_entire_column (colnum, myarr);
  map.Set (myarr, ordering=="NEST" ? NEST : RING);
  }
...
#if _MSC_VER == 1300
  //this section breaks the VS 2003 compiler, replaced by a call to a function which does this assuming a RING structure
...
#else
  out.add_healpix_keys(base.Npix());
#endif
  }
