char *bcrcsid = "RCS: $Id: bary.c,v 1.2 2008/11/01 20:27:19 hirayama Exp $" ;
/*-----------------------------------------------------------------------
 *
 *  bary.c
 *
 ...
 */
  if ( FF = openAFile (TAIUTC) ) {
    ...
    fprintf (stderr, "Could not open leap second file; skipping this step\n") ;
    return 0 ;
  }
 ...
 */
  ...
  /*fprintf(stderr,"xbarycorr: got altitude %f km (scposn=[%f:%f:%f]m)\n",
    (sqrt(pow(scposn[0],2)+pow(scposn[1],2)+pow(scposn[2],2))-6378100.0)/1000.,
    scposn[0],scposn[1],scposn[2]);
  */
/*
 *  Get all positions ---------------------------------------------------
 */
 ...
 */
  total += TTtoTDB (mjdTT)
	     ...
	     + DOT(scposn, vce)/c
	       + 2*msol*log(1+cth);
  /*fprintf(stderr,"total after all propagation effects: %f\n",total);
    fprintf(stderr,"TTtoTDB(mjdTT): %f\n",TTtoTDB(mjdTT));
    fprintf(stderr,"DOT(sourcedir, rca): %f\n",DOT(sourcedir, rca));
    fprintf(stderr,"DOT(scposn, vce)/c: %f\n",DOT(scposn,vce)/c);
    fprintf(stderr,"2*msol*log(1+cth): %f\n",2*msol*log(1+cth));
  */
/*
 *  Sort out time systems -----------------------------------------------
 */
