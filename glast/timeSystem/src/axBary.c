/* $Id: axBary.c,v 1.3 2008/11/01 20:27:19 hirayama Exp $ */
/*--------------------------------------------------------------------
 *
 *    File:        axBary.c
	    ...
	    double ra, double dec, char *refFrame, int debug)
{
  ...
  char *version = "axBary - $Revision: 1.3 $ $Date: 2008/11/01 20:27:19 $" ;
...
/* Masaharu Hirayama 1 November 2008: Commented out variables 'l' in the
  ...
  if ( ( (*inFile) * (*outFile) ) && strcmp(inFile, outFile) ) {
    /* JP of GSFC/HEASARC Added the following, to prevent calling cp. */
    assert(0);
  ...
  if (debug) fprintf(stderr,"axBary: using RA = %f, Dec = %f, frame: %s\n",ra,dec,frame);
   ...
   */
	...
	fprintf (stderr, "%s: HDU %d TSTART = %f, TSTOP = %f\n",
      ...
      /* Added by James Peachey and Masaharu Hirayama 7 October, 2004:
         Test for error here as well. Otherwise, problem converting tstart is not
         detected. */
      if ( error ) {
	fprintf (stderr, "%s: Error %d correcting TSTART in HDU %d\n",
   ...
   */
    ...
    /*if (!strcmp(missionk, "SWIFT"))
      fits_update_key_dbl (fits_file, "UTCFINIT",     0.0, 6,         "&", &error) ;*/
...
*/
  ...
  if ( debug )
    ...
    /* JP of GSFC/HEASARC commented out the following code which deletes
       output file, because there is no check to make sure the output file
       is not the input file, and rm doesn't exist on Windows anyway. */
/*    sprintf (line, "rm %s", outFile) ;
    ...
    fprintf (stderr, "%s: Error exit (error %d); output file %s deleted\n",
	     xbProgName, error, outFile) ;
*/
    /* JP of GSFC/HEASARC added the following warning instead: */
    fprintf (stderr, "%s: Error exit (error %d); please check accessibility and/or content of file %s\n",
	     xbProgName, error, outFile) ;
  }
  exit (error) ;
}
