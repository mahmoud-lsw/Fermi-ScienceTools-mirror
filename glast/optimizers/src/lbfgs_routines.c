/* routines.f -- translated by f2c (version 19960611).
...
actorization in formt;\002,/,\002   refresh the lbfgs memory and restart the\
 iteration.\002)";
    ...
    extern /* Subroutine */ int cauchy_(integer *, doublereal *, doublereal *,
	     doublereal *, integer *, doublereal *, integer *, integer *, 
	    doublereal *, doublereal *, doublereal *, integer *, doublereal *,
	     doublereal *, doublereal *, doublereal *, doublereal *, integer *
	    , integer *, doublereal *, doublereal *, doublereal *, doublereal 
	    *, integer *, doublereal *, doublereal *, integer *, doublereal *,
	     integer *), cmprlb_(integer *, integer *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, doublereal *, integer *,
	     doublereal *, integer *, integer *, integer *, logical *, 
	    integer *), lnsrlb_(integer *, doublereal *, doublereal *, 
	    integer *, doublereal *, doublereal *, doublereal *, doublereal *,
	     doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, integer *, integer *, 
...
/*          restore local variables. */
	...
	gdold = dsave[15];
	dtd = dsave[16];
/*        After returning from the driver go to the point where execut
ion */
/*        is to resume. */
	if (s_cmp(task, "FG_LN", 5L, 5L) == 0) {
	    goto L666;
	}
	if (s_cmp(task, "NEW_X", 5L, 5L) == 0) {
	    goto L777;
	}
	if (s_cmp(task, "FG_ST", 5L, 5L) == 0) {
	    goto L111;
	}
	if (s_cmp(task, "STOP", 4L, 4L) == 0) {
	    if (s_cmp(task + 6, "CPU", 3L, 3L) == 0) {
/*                                          restore the previo
us iterate. */
...
/*                                terminate the algorithm. */
	s_copy(task, "CONVERGENCE: NORM OF PROJECTED GRADIENT <= PGTOL", 60L, 
		48L);
	goto L999;
    }
 ...
 */
    ...
    if (info != 0) {
/*         singular triangular system detected; refresh the lbfgs memo
ry. */
	if (*iprint >= 1) {
	    s_wsfe(&io___66);
	    e_wsfe();
...
L666:
    lnsrlb_(n, &l[1], &u[1], &nbd[1], &x[1], f, &fold, &gd, &gdold, &g[1], &
	    ...
	    csave, &isave[22], &dsave[17], 60L, 60L);
    if (info != 0 || iback >= 20) {
...
/*             abnormal termination. */
	    if (info == 0) {
		info = -9;
/*                restore the actual number of f and g evaluat
ions etc. */
		--nfgv;
		--ifun;
		--iback;
	    }
	    s_copy(task, "ABNORMAL_TERMINATION_IN_LNSRCH", 60L, 30L);
	    ++iter;
	    goto L999;
	} else {
/*             refresh the lbfgs memory and restart the iteration.
 */
	    ...
	    iupdat = 0;
	    updatd = FALSE_;
	    s_copy(task, "RESTART_FROM_LNSRCH", 60L, 19L);
	    timer_(&cpu2);
	    lnscht = lnscht + cpu2 - cpu1;
	    goto L222;
	}
    } else if (s_cmp(task, "FG_LN", 5L, 5L) == 0) {
/*          return to the driver for calculating f and g; reenter at 6
66. */
	goto L1000;
    } else {
...
/* Computing MAX */
    ...
    if (fold - *f <= tol * ddum) {
/*                                        terminate the algorithm. */
	s_copy(task, "CONVERGENCE: REL_REDUCTION_OF_F <= FACTR*EPSMCH", 60L, 
		47L);
	if (iback >= 10) {
	    info = -5;
	}
...
/* L20: */
 ...
 its projection.", 58L);
	...
	if (! (*cnstnd)) {
	    s_wsle(&io___82);
	    do_lio(&c__9, &c__1, "This problem is unconstrained.", 30L);
	    e_wsle();
	}
    }
    if (*iprint > 0) {
...
/*       the derivative f1 and the vector p = W'd (for theta = 1). */
	...
	if (*iprint >= 0) {
	    s_wsle(&io___88);
	    do_lio(&c__9, &c__1, "Subgnorm = 0.  GCP = X.", 23L);
	    e_wsle();
	}
...
/*       The smallest of the nbreak breakpoints is in t(ibkmin)=bkmin. */
    if (*theta != 1.) {
/*                   complete the initialization of p for theta not= o
ne. */
...
/*                  is a zero vector, return with the initial xcp as G
CP. */
	if (*iprint > 100) {
	    s_wsfe(&io___105);
	    i__1 = *n;
...
/*     Initialize derivative f2. */
    ...
    *nint = 1;
    if (*iprint >= 99) {
	s_wsle(&io___109);
	do_lio(&c__9, &c__1, "There are ", 10L);
	do_lio(&c__3, &c__1, (char *)&nbreak, (ftnlen)sizeof(integer));
	do_lio(&c__9, &c__1, "  breakpoints ", 14L);
	e_wsle();
    }
...
/*       compute dt = t(nleft) - t(nleft + 1). */
    tj0 = tj;
    if (iter == 1) {
/*         Since we already have the smallest breakpoint we need not d
o */
...
/*         cost of heapsort is avoided. */
	...
	if (iter == 2) {
/*             Replace the already used smallest breakpoint with t
he */
/*             breakpoint numbered nbreak > nlast, before heapsort
 call. */
	    if (ibkmin != nbreak) {
		t[ibkmin] = t[nbreak];
...
/*       reset the corresponding component of d to zero. */
    ...
    if (*iprint >= 100) {
	s_wsle(&io___121);
	do_lio(&c__9, &c__1, "Variable  ", 10L);
	do_lio(&c__3, &c__1, (char *)&ibp, (ftnlen)sizeof(integer));
	do_lio(&c__9, &c__1, "  is fixed.", 11L);
	e_wsle();
    }
    if (nleft == 0 && nbreak == *n) {
/*                                             all n variables are fix
ed, */
/*                                                return with xcp as G
CP. */
	dtm = dt;
	goto L999;
    }
...
L888:
	...
	e_wsle();
	s_wsle(&io___127);
	do_lio(&c__9, &c__1, "GCP found in this segment", 25L);
	e_wsle();
	s_wsfe(&io___128);
	do_fio(&c__1, (char *)&(*nint), (ftnlen)sizeof(integer));
	do_fio(&c__1, (char *)&f1, (ftnlen)sizeof(doublereal));
...
/*     Check the input arguments for errors. */
    ...
    /* Function Body */
    if (*n <= 0) {
	s_copy(task, "ERROR: N .LE. 0", 60L, 15L);
    }
    if (*m <= 0) {
	s_copy(task, "ERROR: M .LE. 0", 60L, 15L);
    }
    if (*factr < 0.) {
	s_copy(task, "ERROR: FACTR .LT. 0", 60L, 19L);
    }
...
/*                                                   return */
	    s_copy(task, "ERROR: INVALID NBD", 60L, 18L);
	    *info = -6;
	    *k = i__;
	}
	if (nbd[i__] == 2) {
	    if (l[i__] > u[i__]) {
/*                                    return */
		s_copy(task, "ERROR: NO FEASIBLE SOLUTION", 60L, 27L);
		*info = -7;
		*k = i__;
	    }
	}
...
/* L22: */
	...
	if (*iprint >= 99) {
	    ...
	    do_lio(&c__9, &c__1, " variables leave; ", 18L);
	    do_lio(&c__3, &c__1, (char *)&(*nenter), (ftnlen)sizeof(integer));
	    do_lio(&c__9, &c__1, " variables enter", 16L);
	    e_wsle();
	}
    }
...
/* L24: */
	...
	do_lio(&c__9, &c__1, " variables are free at GCP ", 27L);
	i__1 = *iter + 1;
	do_lio(&c__3, &c__1, (char *)&i__1, (ftnlen)sizeof(integer));
	e_wsle();
    }
...
/*                               Line search is impossible. */
    ...
    dcsrch_(f, gd, stp, &c_b275, &c_b276, &c_b277, &c_b9, stpmx, csave, &
	    isave[1], &dsave[1], 60L);
    *xstep = *stp * *dnorm;
    if (s_cmp(csave, "CONV", 4L, 4L) != 0 && s_cmp(csave, "WARN", 4L, 4L) != 
	    0) {
	s_copy(task, "FG_LNSRCH", 60L, 9L);
	++(*ifun);
	++(*nfgv);
	*iback = *ifun - 1;
	if (*stp == 1.) {
...
/*                            the subspace minimization converged. */
	s_copy(word, "con", 3L, 3L);
    } else if (*iword == 1) {
/*                          the subspace minimization stopped at a bou
nd. */
	s_copy(word, "bnd", 3L, 3L);
    } else if (*iword == 5) {
/*                             the truncated Newton step has been used
. */
	s_copy(word, "TNT", 3L, 3L);
    } else {
	s_copy(word, "---", 3L, 3L);
    }
    if (*iprint >= 99) {
	s_wsle(&io___194);
	do_lio(&c__9, &c__1, "LINE SEARCH", 11L);
	do_lio(&c__3, &c__1, (char *)&(*iback), (ftnlen)sizeof(integer));
	do_lio(&c__9, &c__1, " times; norm of step = ", 23L);
...
L999:
	...
	if (*info != 0) {
	    ...
	    if (*info == -6) {
		s_wsle(&io___214);
		do_lio(&c__9, &c__1, " Input nbd(", 11L);
		do_lio(&c__3, &c__1, (char *)&(*k), (ftnlen)sizeof(integer));
		do_lio(&c__9, &c__1, ") is invalid.", 13L);
		e_wsle();
	    }
	    if (*info == -7) {
		s_wsle(&io___215);
		do_lio(&c__9, &c__1, " l(", 3L);
		do_lio(&c__3, &c__1, (char *)&(*k), (ftnlen)sizeof(integer));
		do_lio(&c__9, &c__1, ") > u(", 6L);
		do_lio(&c__3, &c__1, (char *)&(*k), (ftnlen)sizeof(integer));
		do_lio(&c__9, &c__1, ").  No feasible solution.", 25L);
		e_wsle();
	    }
	    if (*info == -8) {
		s_wsfe(&io___216);
...
/* L70: */
	...
	if (alpha < 1.) {
	    ...
	    s_wsle(&io___247);
	    do_lio(&c__9, &c__1, "SM solution inside the box", 26L);
	    e_wsle();
	}
	if (*iprint > 100) {
	    s_wsfe(&io___248);
...
/*     Initialization block. */
    ...
    /* Function Body */
    if (s_cmp(task, "START", 5L, 5L) == 0) {
/*        Check the input arguments for errors. */
	if (*stp < *stpmin) {
	    s_copy(task, "ERROR: STP .LT. STPMIN", task_len, 22L);
	}
	if (*stp > *stpmax) {
	    s_copy(task, "ERROR: STP .GT. STPMAX", task_len, 22L);
	}
	if (*g >= 0.) {
	    s_copy(task, "ERROR: INITIAL G .GE. ZERO", task_len, 26L);
	}
	if (*ftol < 0.) {
	    s_copy(task, "ERROR: FTOL .LT. ZERO", task_len, 21L);
	}
	if (*gtol < 0.) {
	    s_copy(task, "ERROR: GTOL .LT. ZERO", task_len, 21L);
	}
	if (*xtol < 0.) {
	    s_copy(task, "ERROR: XTOL .LT. ZERO", task_len, 21L);
	}
	if (*stpmin < 0.) {
	    s_copy(task, "ERROR: STPMIN .LT. ZERO", task_len, 23L);
	}
	if (*stpmax < *stpmin) {
	    s_copy(task, "ERROR: STPMAX .LT. STPMIN", task_len, 25L);
	}
...
/*     Test for warnings. */
	...
	s_copy(task, "WARNING: XTOL TEST SATISFIED", task_len, 28L);
    }
    if (*stp == *stpmax && *f <= ftest && *g <= gtest) {
	s_copy(task, "WARNING: STP = STPMAX", task_len, 21L);
    }
    if (*stp == *stpmin && (*f > ftest || *g >= gtest)) {
	s_copy(task, "WARNING: STP = STPMIN", task_len, 21L);
    }
/*     Test for convergence. */
    if (*f <= ftest && abs(*g) <= *gtol * (-ginit)) {
	s_copy(task, "CONVERGENCE", task_len, 11L);
    }
/*     Test for termination. */
    if (s_cmp(task, "WARN", 4L, 4L) == 0 || s_cmp(task, "CONV", 4L, 4L) == 0) 
	    {
	goto L1000;
    }
 ...
 */
/*     sign. The minimum is bracketed. If the cubic step is farther fr
om */
/*     stp than the secant step, the cubic step is taken, otherwise th
e */
...
/* Computing 2nd power */
	...
	*brackt = TRUE_;
/*     Third case: A lower function value, derivatives of the same sig
n, */
/*     and the magnitude of the derivative decreases. */
    } else if (abs(*dp) < abs(*dx)) {
/*        The cubic step is computed only if the cubic tends to infini
ty */
...
/*           A minimizer has not been bracketed. If the cubic step
 is */
/*           farther from stp than the secant step, the cubic step
 is */
/*           taken, otherwise the secant step is taken. */
	    if ((d__1 = stpc - *stp, abs(d__1)) > (d__2 = stpq - *stp, abs(
		    d__2))) {
...
/*     Fourth case: A lower function value, derivatives of the same si
gn, */
